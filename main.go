...
func main() {
	...
	// Initialize fallback state store (必须在数据库初始化后)
	if common.IsFallbackEnabled {
		if err := fallback.InitStateStore(); err != nil {
			logger.SysError("failed to initialize fallback state store: " + err.Error())
			logger.SysError("fallback state will be disabled")
		} else {
			logger.SysLog("fallback state store initialized successfully")
		}
	}

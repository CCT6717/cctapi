...
const SiderBar = () => {
  ...
  const headerButtons = useMemo(() => [
    ...
    {
      text: '回退面板',
      itemKey: 'fallback-dashboard',
      external: true,
      href: '/fallback/dashboard',
      icon: <IconHistogram />,
      className: isAdmin() ? 'semi-navigation-item-normal' : 'tableHiddle'
    },
    {
      text: '监控指标',
      itemKey: 'fallback-metrics',
      external: true,
      href: '/fallback/metrics',
      icon: <IconBox />,
      className: isAdmin() ? 'semi-navigation-item-normal' : 'tableHiddle'
    },
    {
      text: '排序分数',
      itemKey: 'fallback-scores',
      external: true,
      href: '/fallback/scores',
      icon: <IconList />,
      className: isAdmin() ? 'semi-navigation-item-normal' : 'tableHiddle'
    },
    {
      text: '告警历史',
      itemKey: 'fallback-alerts',
      external: true,
      href: '/fallback/alerts',
      icon: <IconHistogram />,
      className: isAdmin() ? 'semi-navigation-item-normal' : 'tableHiddle'
    },
    {
      text: '切换日志',
      itemKey: 'fallback-logs',
      external: true,
      href: '/fallback/logs',
      icon: <IconList />,
      className: isAdmin() ? 'semi-navigation-item-normal' : 'tableHiddle'
        ...
        <div style={{ height: '100%' }}>
          <Nav
            ...
            renderWrapper={({ itemElement, isSubNav, isInSubNav, props }) => {
              if (props.external) {
                return (
                  <a style={{ textDecoration: 'none' }} href={props.href} target="_blank" rel="noopener noreferrer">
                    {itemElement}
                  </a>
                );
              }

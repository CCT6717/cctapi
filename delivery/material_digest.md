...
| 章节 | 内容摘要 |
| --- | --- |
| D4，§1 module 声明 | 模块名保持上游 `github.com/songquanpeng/one-api`（go.mod:1）；`go 1.20`（go.mod:3）（直接引用；与 D1，§6 实际使用 go1.22.12 工具链的差异见 §3 X3） |
...
| X3 | Go 版本口径 | `go 1.20`（模块声明） | D4，§1 module 声明（go.mod:3） | 实际构建使用 go1.22.12 工具链（`D:\ct\tools\go1.22.12\bin\go.exe`） | D1，§6 Current Handoff-验证结果 | go.mod 声明的最低版本与实际工具链版本不同；非直接矛盾，但下游做构建方案时需注意两个口径 |

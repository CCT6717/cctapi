...
| 业务术语 | 业务定义（≤ 30 字） | 应用层核心对象（§3.3 O-xx） | 数据库表名（§4.2） | 状态 |
| --- | --- | --- | --- | --- |
| 虚拟模型 | 一个模型名映射多个真实部署的抽象 | VirtualModelConfig | t_fallback_virtual_model | CURRENT |
| 部署 | 真实上游渠道 + 真实模型 + 配额/能力配置 | DeploymentConfig | t_fallback_deployment | CURRENT |
| 免费池 | 自动同步上游免费供应商的部署集合 | FreeProviderConfig | t_channel（复用）+ t_fallback_deployment | CURRENT |
| 策略排序 | 按 quality/cost/free 策略对部署评分排序 | ScoreWeights / DeploymentPlan | 运行时内存，无持久表 | CURRENT |
| 冷却 | 部署因失败/限额被临时/长期标记不可用 | CooldownPolicy | t_fallback_deployment（状态字段） | CURRENT |
| 用量台账 | 免费供应商按 provider/key/model/period 的 token 计数 | FreeProviderUsageLedger | t_free_provider_usage | CURRENT |
| 切换日志 | 请求级回退事件记录 | FallbackEvent | t_fallback_event_log | CURRENT |
| 告警 | 配额/冷却/全部失败等事件通知 | AlertRecord | t_alert_history | CURRENT |
| 上游供应商 | OpenRouter / Kilo / OVH / Pollinations 等 | UpstreamAdapter | 外部系统，无表 | CURRENT |
...
| 业务域 | 核心场景 | 涉及角色 | 与其他业务域的关系 | 状态 |
| --- | --- | --- | --- | --- |
| 虚拟模型路由与回退 | 模型拦截、策略排序、顺序回退、sticky 复用 | API 消费者、网关管理员 | 上游依赖多协议兼容；下游驱动用量记账与可观测 | CURRENT |
| 免费池路由 | 免费供应商注册表同步、动态目录、用量台账 | 网关管理员、运营 / SRE | 被虚拟模型路由调用；依赖上游供应商适配 | CURRENT |
| 多协议兼容 | Chat Completions / Responses / Anthropic Messages 转换与转发 | API 消费者 | 被虚拟模型路由调用；向下依赖上游供应商适配 | CURRENT |
| 用量记账与告警 | token 计数、配额预检、冷却/配额/失败告警 | 运营 / SRE、网关管理员 | 消费虚拟模型路由与免费池路由事件；依赖配置热重载 | CURRENT |
| 可观测性 | 面板展示、Prometheus 指标、切换日志、告警历史 | 网关管理员、运营 / SRE | 消费 SQLite 状态与上游供应商运行时数据 | CURRENT |
| 基础能力（配置/限流/鉴权） | 配置热重载、用户/令牌鉴权、可选 Redis 限流 | 网关管理员、平台团队 | 被全部业务能力域复用 | CURRENT |
...
| 限界上下文 | 对应业务域 | 域类型 | 覆盖的核心业务实体 | 上下文边界（属于本域 vs 不属于） | 状态 |
| --- | --- | --- | --- | --- | --- |
| 虚拟模型路由上下文 | 虚拟模型路由与回退 | 核心域 | 虚拟模型、部署、策略排序、冷却状态、sticky | 属于：策略排序/冷却/sticky；不属于：上游供应商协议细节（归适配上下文） | CURRENT |
| 免费池路由上下文 | 免费池路由 | 核心域 | 免费供应商注册表、目录快照、用量台账 | 属于：provider 注册表/同步/台账；不属于：上游供应商鉴权（归适配上下文） | CURRENT |
| 协议适配上下文 | 多协议兼容 | 支撑域 | Chat/Responses/Anthropic 请求转换 | 属于：请求/响应转换；不属于：上游供应商真实模型能力（归适配上下文） | CURRENT |
| 用量告警上下文 | 用量记账与告警 | 支撑域 | 用量台账、配额、告警记录 | 属于：token 计数/配额预检/告警；不属于：上游供应商计费（归外部） | CURRENT |
| 可观测上下文 | 可观测性 | 支撑域 | 面板、Prometheus 指标、切换日志、评分历史 | 属于：运行时状态展示/日志；不属于：外部 APM 存储（归外部） | CURRENT |
| 用户令牌上下文 | 基础能力（配置/限流/鉴权） | 通用域 | 用户、令牌、渠道、权限 | 属于：沿用 one-api 模型；不属于：业务侧虚拟模型策略 | CURRENT |

#### 2.2.2 上下文映射（Context Mapping） (CURRENT)

| 关系编号 | 上游上下文 | 下游上下文 | 关系模式 | 同步方式 | 选择理由 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| C-01 | 用户令牌上下文 | 虚拟模型路由上下文 | Shared Kernel | 复用 Channel / Token 表与权限校验 | 用户/渠道/令牌模型是 one-api 既有公共概念，多域必须一致 | CURRENT |
| C-02 | 上游 LLM 供应商（E-01~E-04） | 协议适配上下文 | ACL 防腐层 | 适配器模式（relay/adaptor/*） | 外部 OpenAI/Anthropic 协议不污染本域模型 | CURRENT |
| C-03 | 虚拟模型路由上下文 | 用量告警上下文 | Customer/Supplier | 同步事件回调 + SQLite 写 | 用量告警上下文消费路由结果，路由上下文主导数据产出节奏 | CURRENT |
| C-04 | 免费池路由上下文 | 虚拟模型路由上下文 | Customer/Supplier | 内存部署列表同步 | 免费池为路由上下文提供候选部署集合 | CURRENT |
| C-05 | 可观测上下文 | 全部业务能力上下文 | Published Language | Prometheus /metrics + 结构化日志 | 多下游消费统一的运行时事件与指标 | CURRENT |
| C-06 | 上游 LLM 供应商（OpenRouter） | 免费池路由上下文 | Conformist | 直接遵循 OpenRouter 免费层限速语义 | 无议价能力，免费层 429/日配额分档为外部事实约束 | CURRENT |

#### 2.2.3 跨域协作原则 (CURRENT)

| 原则编号 | 原则 | 状态 |
| --- | --- | --- |
| P-01 | 核心域 → 支撑域 / 通用域：禁止反向依赖（虚拟模型路由上下文不依赖用量告警上下文的内部模型） | CURRENT |
| P-02 | 核心域之间：禁止直接耦合，必须通过事件 / API 解耦（虚拟模型路由上下文与免费池路由上下文通过部署列表接口交互） | CURRENT |
| P-03 | 对接外部不可控系统：必须建 ACL 防腐层，禁止把外部模型贯穿到本域（Responses / Anthropic 转换在 relay/model 边界完成） | CURRENT |
...
| 编号 | 一级功能名 | 子功能描述 | 对应业务域 | 实现状态 | 状态 |
| --- | --- | --- | --- | --- | --- |
| F1 | 虚拟模型路由与回退 | — | 虚拟模型路由与回退 | — | CURRENT |
| F1.1 | — | 模型拦截与策略排序 | 虚拟模型路由与回退 | 完整实现 | CURRENT |
| F1.2 | — | 多真实部署顺序回退 | 虚拟模型路由与回退 | 完整实现 | CURRENT |
| F1.3 | — | sticky 路由与 warm-up | 虚拟模型路由与回退 | 完整实现 | CURRENT |
| F2 | 冷却与配额 | — | 虚拟模型路由与回退 | — | CURRENT |
| F2.1 | — | 错误分类差异化冷却（429/5xx/401/403/配额） | 虚拟模型路由与回退 | 完整实现 | CURRENT |
| F2.2 | — | RPM/RPD/TPM/TPD 四维限额预检 | 虚拟模型路由与回退 | 完整实现 | CURRENT |
| F3 | 免费池路由 | — | 免费池路由 | — | CURRENT |
| F3.1 | — | 免费供应商注册表同步 | 免费池路由 | 完整实现 | CURRENT |
| F3.2 | — | 免费池用量台账原子 upsert | 免费池路由 | 完整实现 | CURRENT |
| F4 | 多协议兼容 | — | 多协议兼容 | — | CURRENT |
| F4.1 | — | Chat Completions 兼容 | 多协议兼容 | 完整实现 | CURRENT |
| F4.2 | — | Responses → Chat 转换 | 多协议兼容 | 完整实现 | CURRENT |
| F4.3 | — | Anthropic Messages 兼容 | 多协议兼容 | 完整实现 | CURRENT |
| F5 | 用量记账与告警 | — | 用量记账与告警 | — | CURRENT |
| F5.1 | — | Prometheus 指标与告警 | 用量记账与告警 | 完整实现 | CURRENT |
| F5.2 | — | 切换日志与运行时状态面板 | 用量记账与告警、可观测 | 完整实现 | CURRENT |
| F6 | 配置热重载 | — | 基础能力 | — | CURRENT |
| F6.1 | — | POST /api/fallback/config/reload | 基础能力 | 完整实现 | CURRENT |
| F6.2 | — | strategy + pools 基线格式与 legacy 兼容 | 基础能力 | 完整实现 | CURRENT |
| F7 | 可观测性面板 | — | 可观测 | — | CURRENT |
| F7.1 | — | /fallback/status 管理面板 | 可观测 | 完整实现 | CURRENT |
| F7.2 | — | /fallback/free-pool 免费池管理面板 | 可观测、免费池路由 | 完整实现 | CURRENT |
| F8 | 用户/令牌鉴权 | — | 基础能力 | — | CURRENT |
| F8.1 | — | 沿用 one-api TokenAuth + AdminAuth | 基础能力 | 完整实现 | CURRENT |
...
| 编号 | 本期必做的事项 | 状态 |
| --- | --- | --- |
| F1 | 虚拟模型拦截与策略排序 | CURRENT |
| F2 | 多真实部署顺序回退 | CURRENT |
| F3 | 错误分类与差异化冷却 | CURRENT |
| F4 | 并发槽与配额预检 | CURRENT |
| F5 | sticky 路由与 warm-up | CURRENT |
| F6 | 免费池注册表与动态目录同步 | CURRENT |
| F7 | 免费池用量台账 | CURRENT |
| F8 | Chat Completions 兼容 | CURRENT |
| F9 | Responses 兼容 | CURRENT |
| F10 | Anthropic Messages 兼容 | CURRENT |
| F11 | Prometheus 指标与告警 | CURRENT |
| F12 | 切换日志与运行时状态面板 | CURRENT |
| F13 | 配置热重载与 strategy + pools 基线格式 | CURRENT |
| N1 | 支持单实例自托管，SQLite 为主存 | CURRENT |
| N2 | 可选 Redis 限流，无 Redis 时内存限流兜底 | CURRENT |

**Out-of-Scope（本期不做）**：

| 编号 | 不做的事项 | 不做的原因 | 未来归属 | 状态 |
| --- | --- | --- | --- | --- |
...
| 优先级 | 质量目标 | 业务驱动 | 受影响的关键章节 | 状态 |
| --- | --- | --- | --- | --- |
| Q1 | 高可用：单上游故障时自动切换，虚拟模型请求成功率 ≥ 99%（排除上游免费层限额） | 用户 Top1 痛点：单上游故障导致请求失败 | §3.2 M1 / §5.2 / §8.4 | TARGET |
| Q2 | 低延迟：管理端面板 P99 ≤ 2s；LLM 转发 P99 ≤ 60s（含上游推理时间） | 用户体验阈值；管理端日常运营 | §3.2 / §5.3 | TARGET |
| Q3 | 可观测性：切换日志、用量台账、告警 100% 留痕 | 合规与故障定位需求 | §8.1 / §8.2 / §8.4 | TARGET |
| Q4 | 可维护性：单实例自托管，SQLite 零依赖，新人 1 周可本地运行 | 团队规模与部署形态 | §3.1 / §5.1 | TARGET |
| Q5 | 兼容性：Chat / Responses / Anthropic 三协议非流式 + 流式稳定运行 | 已验收功能需保持 100% | §3.2 M3 / §4.2 | TARGET |

**可验证质量场景**：

| 场景编号 | 关联目标 | 触发源 | 触发条件 | 期望响应 | 度量指标 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| QS-01 | Q1 | 单上游故障 | 主部署返回 429/5xx | 自动切换至备用部署 | 端到端成功率 ≥ 99%；切换时延 ≤ 1s | TARGET |
| QS-02 | Q2 | 管理端访问 | 面板加载 100 并发 | 正常返回 | 面板 P99 ≤ 2s；API P99 ≤ 60s | TARGET |
| QS-03 | Q3 | 生产请求 | 任意虚拟模型请求 | 切换事件落 SQLite；告警事件写历史 | 日志完整率 100%；告警无遗漏 | TARGET |
| QS-04 | Q5 | 协议回归测试 | 三协议非流式/流式各 100 请求 | 全部通过 | 验收通过率 100% | TARGET |
...
| 层级 | 含义 | 故障态下的处置方向 | 用户感知 | 状态 |
| --- | --- | --- | --- | --- |
| L0 核心业务 | 系统的生命线，挂掉等于业务停摆 | 不降级；优先消耗所有资源保障 | 无 / 极轻微 | CURRENT |
| L1 重要业务 | 不影响主链路但用户高频使用 | 限流 + 排队 + 异步化 | 变慢 / 排队提示 | CURRENT |
| L2 辅助业务 | 提升体验但非必要 | 关闭 / 返回兜底数据 | 部分功能不可用 + 友好提示 | CURRENT |
| L3 附加业务 | 长尾 / 数据类 / 统计类 | 直接关闭 + 异步补偿 | 通常无感 | CURRENT |

**业务降级清单**：

| 编号 | 关联功能（F-xx） | 触发条件 | 降级动作 | 用户感知 | 决策类型 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| BD-01 | F1（虚拟模型路由） | 全部部署失败 | 返回标准错误响应 | 请求暂时无法完成，请稍后重试 | 自动 | CURRENT |
| BD-02 | F3.1（免费池同步） | 上游目录 HTTP 超时 | 使用本地快照继续服务；后台重试 | 无感知（仍可路由） | 自动 | CURRENT |
| BD-03 | F5.1（Prometheus 指标） | metrics 端点异常 | 跳过指标上报，不影响主请求 | 无感知 | 自动 | CURRENT |
| BD-04 | F7.1（管理面板） | SQLite 只读或锁定 | 返回静态提示页；读接口可能延迟 | 管理面板加载变慢 | 自动 | CURRENT |
...
| 节点类型 | 节点名 | 与本系统的关系 | 关系语义 | 状态 |
| --- | --- | --- | --- | --- |
| 角色 | API 消费者 | 使用 | 调用 /v1/* 接口 | CURRENT |
| 角色 | 网关管理员 | 使用 | 管理端 /fallback/* 操作 | CURRENT |
| 角色 | 运营 / SRE | 使用 | 查看面板、处理告警 | CURRENT |
| 上游平台 | E-01 OpenRouter | 本系统 → 调用 | 免费层 / 付费层 LLM 推理 | CURRENT |
| 上游平台 | E-02 Kilo | 本系统 → 调用 | keyless 免费 LLM 推理 | CURRENT |
| 上游平台 | E-03 OVH | 本系统 → 调用 | keyless 免费 LLM 推理 | CURRENT |
| 上游平台 | E-04 Pollinations | 本系统 → 调用 | keyless 免费 LLM 推理 | CURRENT |
| 上游平台 | E-05 Redis（可选） | 本系统 → 调用 | 分布式限流后端 | CURRENT |
| 外部业务系统 | E-06 上游 one-api 用户/令牌模型 | 复用 | 鉴权、权限、渠道模型 | CURRENT |
| 下游平台 | 客户端应用 | 使用 | 消费 OpenAI 兼容响应 | CURRENT |
...
| 层次 | 内容 | 数据来源 | 状态 |
| --- | --- | --- | --- |
| 接入层 | 客户端 OpenAI API、管理端 /fallback/status、免费池管理端 /fallback/free-pool | §2.3 用例图角色 | CURRENT |
| 应用层 | M1 虚拟模型路由、M2 免费池路由、M3 多协议兼容、M4 用量记账与告警、M5 可观测性 | §2.1 / §3.2 | CURRENT |
| 中间件层 | C-01 SQLite、C-02 可选 Redis、C-03 Prometheus 指标端点 | §3.1.3 云组件清单 | CURRENT |
| 集成对接层 | E-01~E-04 上游 LLM 供应商、E-05 可选 Redis、E-06 one-api 用户/令牌模型 | §3.1.3 外部依赖清单 | CURRENT |

#### 3.1.3 集成架构 (CURRENT)

##### 外部依赖清单

| ID | 外部系统 | 归属团队 / 供应商 | 类别 | 使用方式 | 集成方式 | 协议 / 版本 | 功能定位（≤ 30 字） | 联系人 | 状态 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| E-01 | OpenRouter | OpenRouter Inc. | 第三方业务平台 | 消费 | HTTPS REST | OpenAI 兼容 | 免费/付费 LLM 推理上游 | 外部供应商 | CURRENT |
| E-02 | Kilo | Kilo | 第三方业务平台 | 消费 | HTTPS REST | OpenAI 兼容 | keyless 免费 LLM 上游 | 外部供应商 | CURRENT |
| E-03 | OVH | OVHcloud | 第三方业务平台 | 消费 | HTTPS REST | OpenAI 兼容 | keyless 免费 LLM 上游 | 外部供应商 | CURRENT |
| E-04 | Pollinations | Pollinations.AI | 第三方业务平台 | 消费 | HTTPS REST | OpenAI 兼容 | keyless 免费 LLM 上游 | 外部供应商 | CURRENT |
| E-05 | Redis（可选） | 基础设施 | 缓存 | 消费 | Redis 协议 | Redis 6+ | 分布式限流后端 | 平台团队 | CURRENT |
| E-06 | one-api 用户/令牌/渠道模型 | cctapi 既有 | 身份认证 / 业务协作 | 复用 | 函数调用 | 内部 | 鉴权、权限、渠道映射 | 后端团队 | CURRENT |

##### 云组件 / 基础设施清单

| ID | 组件类别 | 选型 | 部署形态 | 规格量级 | 容量预估 | 提供方 / SLA | 用途 | 状态 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| C-01 | 关系型数据库 | SQLite 3 + mattn/go-sqlite3 v1.14.22 | 单文件，随进程启动 | 2C8G 宿主机，DB 文件 ≤ 10GB | 100 万条切换日志/年 | 进程内；备份依赖文件复制 | 业务主数据、状态、台账 | CURRENT |
| C-02 | 缓存（可选） | Redis 6.0+ / go-redis v8.11.5 | 单实例 / 主从 | 2C4G（可选） | 限流计数 Key 数不超过 1 万 | 可选组件 | 分布式限流 | CURRENT |
| C-03 | 监控告警 | Prometheus 2.x + 文本指标 | 单进程 /metrics | 节点级采集 | 请求量/回退/用量指标 | 外部 Prometheus 拉取 | 指标采集 | CURRENT |
| C-04 | 日志服务 | 本地文件 + 可选外部日志系统 | 单文件滚动 | 日志目录按配置保留 | 日增 ≤ 1GB | 本地文件系统 | 应用日志、审计日志 | CURRENT |
...
| 技术分类 | 选型 | 版本 | 备注 / 选型理由 | 状态 |
| --- | --- | --- | --- | --- |
| 后端语言 | Go | go 1.20（声明）；go1.22.12 工具链 | 单二进制、零依赖、高并发；团队已用 Go 实现 cctapi 全部底座。不选 Python/LiteLLM 是因为需要单实例零依赖。 | CURRENT |
| 后端框架 | Gin | v1.10.0 | cctapi 沿用 one-api 的 Gin 中间件链；生态成熟、性能高。不选 Echo/Fiber 是因为继承既有代码与中间件。 | CURRENT |
| 持久层（ORM） | GORM | v1.25.10 | 沿用 one-api 的 GORM 模型与迁移；支持 SQLite/MySQL/PostgreSQL。不选 sqlx 是因为需要复用 Channel/Token/User 模型。 | CURRENT |
| 关系型存储 | SQLite | 3.x（mattn/go-sqlite3 v1.14.22） | 单实例自托管零依赖；cctapi 已生产验证。不选 PostgreSQL/MySQL 作为默认是因为用户裁决单实例自托管。 | CURRENT |
| 缓存（可选） | Redis | 6.0+；go-redis v8.11.5 | 仅作分布式限流后端；无 Redis 时内存限流兜底。不选 Memcached 是因为 go-redis 与现有 Lua 限流脚本已集成。 | CURRENT |
| 内存缓存 | go-cache | v2 兼容 | 进程内限流 fallback、运行时状态。不选 ristretto 是因为简单场景 go-cache 足够。 | CURRENT |
| 消息队列 | 无 | — | 单实例无 MQ；事件直接函数回调或内存队列。不选 Kafka/RabbitMQ 是因为零依赖目标。 | CURRENT |
| API 通信 | REST | OpenAI 兼容 REST | 对外暴露 OpenAI 兼容 API；内部用 Gin Handler。不选 gRPC 是因为客户端生态要求 OpenAI 兼容。 | CURRENT |
| 前端框架 | React + JavaScript | 18.2 | cctapi 默认主题已用 React 18 + Vite；不选 Vue/Angular 是因为继承现有代码。 | CURRENT |
| 前端构建 | Vite | v6.4.3 | 已从 CRA 迁移；构建速度快、ESM 按需加载。不选 Webpack 是因为已迁移且零 ESLint 警告。 | CURRENT |
| 前端 UI 组件 | Ant Design + Semantic UI React | antd ^6.5.0；semantic-ui-react ^2.1.3 | 现有面板混合使用；保留以减少重写。不选纯 Tailwind 是因为现有组件依赖 Semantic UI。 | CURRENT |
| 图表 | Recharts | ^2.15.1 | 评分趋势图已用 Recharts。不选 ECharts 是因为现有实现足够。 | CURRENT |
| 认证方案 | JWT + Session | 沿用 one-api | TokenAuth 中间件校验 API Key；AdminAuth 校验管理员权限。不选 OAuth2 是因为自托管场景无需外部 IDP。 | CURRENT |
| 测试框架 | Go testing + testify；Vitest + Playwright | testify 最新；Vitest 3.2.7；Playwright ^1.61.1 | 后端用 testify 与 race 检测；前端用 Vitest + Playwright E2E。不选 Jest 是因为已迁移 Vitest。 | CURRENT |
...
| 编号 | 模块名 | 业务域（§2.1） | 模块负责人 | 关联子领域 | 状态 |
| --- | --- | --- | --- | --- | --- |
| M1 | 虚拟模型路由与回退 | 虚拟模型路由与回退 | @后端团队 | 部署计划、策略排序、冷却、sticky | CURRENT |
| M2 | 免费池路由 | 免费池路由 | @后端团队 | provider 注册表、同步、台账 | CURRENT |
| M3 | 多协议兼容 | 多协议兼容 | @后端团队 | Chat/Responses/Anthropic 转换 | CURRENT |
| M4 | 用量记账与告警 | 用量记账与告警 | @后端团队 | 用量台账、配额、告警 | CURRENT |
| M5 | 可观测性 | 可观测性 | @后端团队 | 面板、指标、日志、评分历史 | CURRENT |
...
| 状态 | GET | /api/fallback/states | 运行时状态 | — | FallbackStatesVO | 天然 |
| 评分 | GET | /api/fallback/sort/scores | 评分列表 | — | ScoreListVO | 天然 |
| 评分 | GET | /api/fallback/sort/history | 评分历史 | ScoreHistoryQuery | ScoreHistoryVO | 天然 |
| 指标 | GET | /metrics | Prometheus 指标 | — | text/plain | 天然 |
| 日志 | GET | /api/fallback/logs | 切换日志 | FallbackLogQuery | FallbackLogVO | 天然 |
| 告警 | GET | /api/fallback/alert/status | 当前告警 | — | AlertStatusVO | 天然 |
| 告警 | GET | /api/fallback/alert/history | 告警历史 | AlertHistoryQuery | AlertHistoryVO | 天然 |
...
| 维度 | 取值 | 状态 |
| --- | --- | --- |
| 备份方式 | 全量（文件复制）；冷备为主；SQLite 事务中可安全复制 | TARGET |
| 备份位置 | 与生产环境物理隔离的目录或对象存储 | TARGET |
| 保留策略 | 全量保留 30 天；日志保留 180 天；告警历史保留 90 天 | TARGET |
| RPO（数据丢失容忍） | ≤ 24 小时（依赖每日备份频率） | TARGET |
| RTO（恢复时间目标） | ≤ 30 分钟（单实例，重启 + 数据文件恢复） | TARGET |
| 恢复演练 | 频率：每季度；负责人：@平台团队 | TARGET |
...
| SLA 指标项 | 指标定义 | 目标值 | 度量方式 | 状态 |
| --- | --- | --- | --- | --- |
| 系统开放时间 | 对外提供服务的时间窗口 | 7×24 | 业务侧约定 | TARGET |
| 系统可用性 | 月度可用时长 / 月度总时长 | ≥ 99.5%（单实例自托管，不含上游免费层故障） | 监控统计 | TARGET |
| 单接口分位响应时延 | 端到端响应时间 | 管理面板 P99 ≤ 2s；LLM 转发 P99 ≤ 60s（含上游推理） | APM 采样 | TARGET |
| 故障恢复目标（RTO、RPO） | 故障到恢复的时间 / 数据丢失容忍 | RTO ≤ 30min；RPO ≤ 24h | 演练验证 | TARGET |
| 计划内停机窗口 | 升级 / 维护窗口 | 按需，提前 24h 通知 | 公告 | TARGET |
| 不可用界定 | 何为"不可用" | 错误率 > 5% 持续 5 分钟 或 健康检查连续 3 次失败 | 告警规则 | TARGET |
...
| 能力域 | 选用产品 / 机制 | 作用范围 | 是否启用 | 状态 |
| --- | --- | --- | --- | --- |
| 抗 DDoS | 云厂商 DDoS 防护 / 可选 CDN | 公网入口 | 是（若使用云厂商） | TARGET |
| WAF | 可选云 WAF / Nginx 规则 | Web 应用层 | 是（建议启用） | TARGET |
| 防火墙 / 安全组 | 宿主机防火墙 / 云安全组 | VPC / 子网 / 主机 | 是 | TARGET |
| 主机安全 | 云厂商主机安全 / 自维护 | 全部主机 | 是 | TARGET |
| 容器安全 | 不适用（单二进制，非容器） | — | 不适用 | TARGET |
| 数据安全审计 | SQLite 审计日志 + 应用日志 | 关键数据库 | 是 | TARGET |
| 密钥管理（KMS） | 环境变量 / 文件挂载（自托管无 KMS） | 敏感数据加密 / keys 保护 | 是（环境变量形式） | TARGET |
| 安全运营中心（SOC） | 无独立 SOC；依赖日志告警 | 安全事件监控与响应 | 否（由运维团队承担） | TARGET |
| 堡垒机 | 无；SSH 运维通道 | 运维通道审计 | 否（由基础设施方管理） | TARGET |
...
| 维度 | 取值 | 状态 |
| --- | --- | --- |
| 账号体系 | 自建（沿用 one-api 用户/令牌模型） | TARGET |
| 登录方式 | 账号密码 + API Key | TARGET |
| 多因素认证（MFA） | 否（单实例自托管） | TARGET |
| 密码强度 | 最小长度 6；管理员建议 12+ | TARGET |
| 登录失败锁定 | 连续失败 5 次锁定 15 分钟 | TARGET |
| Token 有效期 | 访问 Token 持久有效；会话 Cookie 按浏览器会话 | TARGET |
| 会话管理 | 单点；JWT / Session 复用 one-api；注销清除 Cookie | TARGET |

#### 7.2.2 数据安全

| 维度 | 取值 | 状态 |
| --- | --- | --- |
| 数据分级 | L1（公开配置）/ L2（运行时状态）/ L3（上游 API keys）/ L4（用户令牌） | TARGET |
| 传输加密 | TLS 强制（生产环境） | TARGET |
| 存储加密 | 上游 API keys 不落地 SQLite；存环境变量 / 配置文件；SQLite 文件由文件系统权限保护 | TARGET |
| 敏感字段清单 | 上游 API keys（L3）/ 用户令牌（L4） | TARGET |
| 数据导出与共享 | 导出需管理员权限；敏感字段脱敏 | TARGET |
...
| 维度 | 取值 | 状态 |
| --- | --- | --- |
| 权限模型 | RBAC0（管理员 / 普通用户） | TARGET |
| 多租户隔离 | 无多租户；单实例用户/令牌隔离 | TARGET |
| 越权防护 | 列表与详情接口必须校验数据归属；管理端接口必须 AdminAuth | TARGET |
| 接口级权限 | 与 §3.5 调用契约对齐：管理接口需 admin；API 接口需有效 token | TARGET |
...
| Dashboard 名 | 受众 | 核心指标 | 刷新频率 | 状态 |
| --- | --- | --- | --- | --- |
| 业务大盘 | 产品 / 业务 | 虚拟模型请求数、回退次数、成功率、免费池用量 | 1min | CURRENT |
| 服务健康大盘 | 研发 / SRE | RED（Rate / Errors / Duration）、P95/P99 延迟 | 30s | CURRENT |
| 中间件大盘 | SRE / 运维 | SQLite 查询耗时、Redis 命中率、限流触发次数 | 30s | CURRENT |
| 容量大盘 | SRE | CPU、内存、磁盘、SQLite 文件大小、日志目录大小 | 1min | CURRENT |
| 业务预警大盘 | On-call | 当前活跃告警（冷却、耗尽、全部失败、配额预警） | 实时 | CURRENT |
| 部署状态大盘 | 网关管理员 | 各部署状态、成功率、错误率、冷却数、耗尽数 | 30s | CURRENT |
...
| 编号 | 级别 | 触发条件 | 维度 | Owner | Runbook | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| AL-01 | P0 | 全部上游失败率 > 5% 持续 5min | 业务可用性 | @后端团队 | docs/runbook/AL-01-all-upstream-failed.md | TARGET |
| AL-02 | P0 | 进程健康检查连续 3 次失败 | 系统健康 | @运维团队 | docs/runbook/AL-02-process-down.md | TARGET |
| AL-03 | P1 | 管理面板 P99 > 2s 持续 5min | 接口延迟 | @后端团队 | docs/runbook/AL-03-panel-latency.md | TARGET |
| AL-04 | P1 | 冷却部署数 > 总部署数 50% | 业务状态 | @后端团队 | docs/runbook/AL-04-too-many-cooling.md | TARGET |
| AL-05 | P2 | 单部署连续 5 次健康检查失败 | 单点异常 | @后端团队 | docs/runbook/AL-05-deployment-unhealthy.md | TARGET |
| AL-06 | P2 | SQLite 写入错误率 > 1% | 数据库健康 | @后端团队 | docs/runbook/AL-06-sqlite-error.md | TARGET |
| AL-07 | P3 | 磁盘容量 > 80% | 资源水位 | @运维团队 | docs/runbook/AL-07-disk-capacity.md | TARGET |
| AL-08 | P3 | 日志目录容量 > 80% | 资源水位 | @运维团队 | docs/runbook/AL-08-log-capacity.md | TARGET |
